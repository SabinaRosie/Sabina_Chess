import 'dart:async';
import 'dart:convert';
import 'package:chess_demo_sabina/core/services/api_service.dart';
...
class NotificationService with WidgetsBindingObserver {
  ...
  /// 🔹 Ping the server every 4 minutes to prevent HF Spaces cold starts
  void _startKeepAlive() {
    _keepAliveTimer?.cancel();
    _keepAliveTimer = Timer.periodic(const Duration(minutes: 4), (timer) {
      ApiService.keepAlive();
    });
    // Also ping immediately on app start
    ApiService.keepAlive();
  }
  ...
  void _initNotifications() async {
    ...
    if (stream != null) {
      _isWsConnected = true;
      // 🔹 Disable polling when WebSocket is alive (avoid redundant traffic)
      ...
      _wsSubscription = stream.listen(
        ...
        onDone: () {
          _isWsConnected = false;
          _startPolling(); // Re-enable polling as fallback
          _reconnect();
        },
        onError: (e) {
          _isWsConnected = false;
  ...
  void _startPolling() {
    ...
    _pollingTimer = Timer.periodic(const Duration(seconds: 30), (timer) async {
      if (_isWsConnected) {
        timer.cancel(); // Stop polling once WS reconnects
        return;
      }
      if (!_isIncomingDialogShown) {
        final result = await SignalingService.checkIncoming();
